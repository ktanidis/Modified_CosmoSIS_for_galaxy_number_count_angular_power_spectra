...
def compute_gaussian_covariance(sky_area, get_theory_cl, block, AB, CD):
    ...
    # galaxy_galcl_emode_fourier, galaxy_position_fourier
    A = AB.type1.name
    B = AB.type2.name
    C = CD.type1.name
    D = CD.type2.name
    ...
    for X in (A, B, C, D):
        assert X in ["galaxy_galcl_emode_fourier",
                     "galaxy_position_fourier"], "Have not yet coded up gaussian covariance for spectra other than pos-pos, galcl-galcl, galcl-pos"
