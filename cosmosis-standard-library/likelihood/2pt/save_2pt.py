...
def setup(options):
    ...
    if make_covariance:
        config['number_density_galcl_bin'] = get_arr(
            "number_density_galcl_bin")
    ...
    config['galcl_nz'] = options.get_string(option_section, "galcl_nz_name").upper()
    ...
    #upload my covariance
    #covari = options[option_section, "covfile"]
    #cuvar = np.loadtxt(covari)
    #config['cuvar'] = cuvar
...
class ObservedClGetter(object):
    def __init__(self, number_density_galcl_bin, number_density_lss_bin, sigma_e_bin):
        self.number_density_galcl_bin = convert_nz_steradian(
            number_density_galcl_bin)
    ...
    def lookup(self, block, A, B, i, j, ell):
        ...
        #print ("Looking noise of EUCLID",1.0/self.number_density_lss_bin[0])
        #print ("Looking noise of SKA1",1.0/self.number_density_lss_bin[1])
        # For galcl-galcl the noise component is sigma^2 / number_density_bin
        # and for position-position it is just 1/number_density_bin
        if (A == B) and (A == twopoint.Types.galaxy_galcl_emode_fourier.name) and (i == j):
            #noise = self.sigma_e_bin[i - 1]**2 / self.number_density_galcl_bin[i - 1]
            noise = 1.0 / self.number_density_galcl_bin[i - 1]
            obs_c_ell += noise
...
def covmat_from_block(block, spectra, sky_area, number_density_galcl_bin, number_density_lss_bin, sigma_e_bin):
#def covmat_from_block(block, spectra, sky_area, number_density_galcl_bin, number_density_lss_bin, sigma_e_bin, cuvar):
    getter = ObservedClGetter(number_density_galcl_bin,
    ...
    #take this instead
    #nll = sum(lengths)
    #assert cuvar.shape==(nll,nll)

    #assert cuvar.shape==(522,522)

    info = twopoint.CovarianceMatrixInfo("COVMAT", names, lengths, C)
    #info = twopoint.CovarianceMatrixInfo("COVMAT", names, lengths, cuvar)
    return info
...
def execute(block, config):
    ...
    need_lens_nz = False
    if fourier_space and block.has_section(names.galcl_cl):
        name = "galcl_cl"
        types = (twopoint.Types.galaxy_galcl_emode_fourier,
                 twopoint.Types.galaxy_galcl_emode_fourier)
        kernels = (galcl_nz, galcl_nz)
        s = spectrum_measurement_from_block(
            block, name, name, types, kernels, angle_sample, real_space)
        print(" - saving galcl_cl")
        need_source_nz = True
        spectra.append(s)

    if fourier_space and block.has_section("galaxy_galcl_cl"):
        name = "galaxy_galcl_cl"
        types = (twopoint.Types.galaxy_position_fourier,
                 twopoint.Types.galaxy_galcl_emode_fourier,)
        kernels = (position_nz, galcl_nz)
        s = spectrum_measurement_from_block(
            block, name, name, types, kernels, angle_sample, real_space)
        print(" - saving galaxy_galcl_cl")
        need_source_nz = True
        need_lens_nz = True
        spectra.append(s)
    ...
    if real_space and block.has_section("galcl_xi"):
        types = (twopoint.Types.galaxy_galcl_plus_real,
                 twopoint.Types.galaxy_galcl_plus_real)
        kernels = (galcl_nz, galcl_nz)
        s = spectrum_measurement_from_block(
            block, "galcl_xi", "xip", types, kernels, angle_sample, real_space)
        print(" - saving xi_plus")
        need_source_nz = True
        spectra.append(s)

        name = "galcl_xi"
        types = (twopoint.Types.galaxy_galcl_minus_real,
                 twopoint.Types.galaxy_galcl_minus_real)
        kernels = (galcl_nz, galcl_nz)
        s = spectrum_measurement_from_block(
            block, "galcl_xi", "xim", types, kernels, angle_sample, real_space)
        need_source_nz = True
        print(" - saving xi_minus")
        spectra.append(s)

    if real_space and block.has_section("galaxy_galcl_xi"):
        types = (twopoint.Types.galaxy_position_real,
                 twopoint.Types.galaxy_galcl_plus_real)
        kernels = (position_nz, galcl_nz)
        s = spectrum_measurement_from_block(
            block, "galaxy_galcl_xi", "gammat", types, kernels, angle_sample, real_space)
        print(" - saving gammat")
        need_source_nz = True
        need_lens_nz = True
        spectra.append(s)
    ...
    if make_covariance:
        covmat_info = covmat_from_block(
            block, spectra, survey_area, number_density_galcl_bin, number_density_lss_bin, sigma_e_bin)
        #covmat_info = covmat_from_block(
        #    block, spectra, survey_area, number_density_galcl_bin, number_density_lss_bin, sigma_e_bin, cuvar)
    else:
        covmat_info = None
    ...
    if need_source_nz:
        ...
        kernels.append(nz_from_block(block, galcl_nz, config['prefix_nz_section']))
    if need_lens_nz and (galcl_nz != position_nz):
