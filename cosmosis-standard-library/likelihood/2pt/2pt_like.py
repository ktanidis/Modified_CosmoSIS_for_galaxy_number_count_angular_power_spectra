...
class TwoPointLikelihood(GaussianLikelihood):
    ...
    def build_data(self):
        ...
        if self.gaussian_covariance:
            ...
            self.number_density_galcl_bin = get_arr("number_density_galcl_bin")
    ...
    def extract_spectrum_prediction(self, block, spectrum):
        ...
        # types of spectra: e.g. galcl-galcl, pos-pos, galcl-pos.
        ...
        # e.g. "galcl_cl_des" instead of just "galcl_cl".
        section += self.suffix
    ...
    def _lookup_theory_cl(self, block, A, B, i, j, ell):
        ...
        if (A == B) and (A == twopoint.Types.galaxy_galcl_emode_fourier.name) and (i == j):
            if i > len(self.number_density_galcl_bin) or i > len(self.sigma_e_bin) or is_default(self.sigma_e_bin) or is_default(self.number_density_galcl_bin):
                raise ValueError(
                    "Not enough number density bins for galcl specified")
            #noise = self.sigma_e_bin[i - 1]**2 / \
            noise = 1.0 / \
                convert_nz_steradian(self.number_density_galcl_bin[i - 1])
            obs_cl += noise
