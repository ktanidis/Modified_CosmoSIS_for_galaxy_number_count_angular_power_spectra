...
def setup(options):
    ...
    if des_fmt:
        ...
        nz = len(data_full[0])
	#nbin = len(data_full)-2 #length of all data lines minus the first which is d and the second z 
	nbin = len(data_full)-3 #length of all data lines minus the first which is d and the second is f and the third is z 
        d = data_full[1] #take D 
        #b = data_full[0] #take b
        f = data_full[0] #take f
	z = data_full[2] #take z
        if single_bin != -666:
    ...
    return (nz, nbin, f, d, z, n_of_z, f_of_z, d_of_z, output_section)
    #return (nz, nbin, z, n_of_z , b_of_z, output_section)


def execute(block, config):
    (nz, nbin, f, d, z, n_of_z, f_of_z, d_of_z, output_section) = config
    #(nz, nbin, z, n_of_z, b_of_z, output_section) = config

    block[output_section, 'nz'] = nz
    block[output_section, 'nbin'] = nbin
    block[output_section, 'f'] = f_of_z
    block[output_section, 'd'] = d_of_z
    #block[output_section, 'b'] = b_of_z
