...
```
In order to consider or not the density fluctuation, the redshift-space distortion and weak lensing magnification contributions specify the value 1 or 0 respectively in the parameters `DEN, RSD` and `MAG` as seen in the above `values.ini` file. Should you have any inqueries regarding the rest of the CosmoSIS modules and the input cosmological parameters visit <a href="https://bitbucket.org/joezuntz/cosmosis/wiki/Home">**CosmoSIS_Wiki**</a>.
