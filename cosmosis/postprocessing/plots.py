...
class Plots(PostProcessorElement):
    ...
    def finalize(self):
        ...
        if legend:
            ...
            for fig in list(self.figures.values()):
                ...
                pylab.legend(loc=3, prop={'size': 10})
    ...
    def line_color(self):
        ...
        #possible_colors = ['r','crimson','b', 'k', 'g', 'm','y'] #this is for Euclid plots
        #possible_colors = ['b','mediumblue','r', 'k', 'g', 'm','y'] #this is for SKA1 plots
        #possible_colors = ['r','k','b', 'k', 'g', 'm','y'] #this is for Euclid plots
        #possible_colors = ['b','k','r', 'k', 'g', 'm','y'] #this is for SKA1 plots

        possible_colors = ['g','b','r','k', 'c', 'm','y'] #this is for neutrino DESI plots
...
class MetropolisHastingsPlots1D(MetropolisHastingsPlots):
    ...
    def make_1d_plot(self, name):
        ...
        #Choose colors
        possible_colors = ['g','b','r','m','y']
        #possible_colors = ['r','grey','g','m','y']
...
#        elif self.line_color()=='k':
#            dark,light = self.shade_colors()
#            pylab.contourf(x_axis, y_axis, like.T, [level2,level0], colors=[light], alpha=0.2)
#            pylab.contourf(x_axis, y_axis, like.T, [level1,level0], colors=[dark], alpha=0.4)
#        else:
#            dark,light = self.shade_colors()
#            pylab.contourf(x_axis, y_axis, like.T, [level2,level0], colors=[light], alpha=0.4)
#            pylab.contourf(x_axis, y_axis, like.T, [level1,level0], colors=[dark], alpha=1.0)


#for neutrinos DESI 
        elif self.line_color()=='b':
            color = self.line_color()
            cs = pylab.contour(x_axis, y_axis, like.T, [level2,level1], colors=color)
            cs.collections[0].set_label(self.source.label)

        elif self.line_color()=='g':
            color = self.line_color()
            cs = pylab.contour(x_axis, y_axis, like.T, [level2,level1], colors=color)
            cs.collections[0].set_label(self.source.label)

        elif self.line_color()=='k':
            #this is shaded, the other is closed lines
            #dark,light = self.shade_colors()
            #pylab.contourf(x_axis, y_axis, like.T, [level2,level0], colors=[light], alpha=0.3)
            #pylab.contourf(x_axis, y_axis, like.T, [level1,level0], colors=[dark], alpha=0.7)
            color = self.line_color()
            cs = pylab.contour(x_axis, y_axis, like.T, [level2,level1], colors=color)
            cs.collections[0].set_label(self.source.label)
        else:
...
#            pylab.contourf(x_axis, y_axis, like.T, [level1,level0], colors=[dark], alpha=1.0)
        ...
        if name1=="cosmological_parameters--omega_m" and name2=="cosmological_parameters--h0":
#            pylab.xlim(0.25,0.385)
#            pylab.ylim(0.62,0.84)
#            pylab.ylim(0.5,1.0)
#            pylab.xlim(0.2,0.4)
            pylab.ylabel(r'$h$',fontsize=25)
            pylab.xlabel(r'$\Omega_m$',fontsize=25)
            pylab.plot(0.3089,0.6774,'wP',ms=10.5) 

        if name1=="cosmological_parameters--sigma8_input" and name2=="cosmological_parameters--h0":
#            pylab.xlim(0.55,1.2)
#            pylab.ylim(0.5,1.0)
            pylab.ylabel(r'$h$',fontsize=25)
            pylab.xlabel(r'$\sigma_8$',fontsize=25)
#            pylab.xlim(0.775,0.85)
#            pylab.ylim(0.62,0.84)
##            pylab.xlim(0.5,1.2)
##            #pylab.xlim(0.76,0.9)
##            #pylab.ylim(0.635,0.73) 
            pylab.plot(0.8159,0.6774,'wP',ms=10.5) 

        if name1=="cosmological_parameters--sigma8_input" and name2=="cosmological_parameters--omega_m":
            pylab.ylabel(r'$\Omega_m$',fontsize=25)
            pylab.xlabel(r'$\sigma_8$',fontsize=25)
#            pylab.xlim(0.55,1.2)
#            pylab.ylim(0.2,0.4)
#            pylab.xlim(0.775,0.85)
#            pylab.xlim(0.5,1.2)
#            #pylab.xlim(0.76,0.9)
#            #pylab.ylim(0.285,0.325)
            pylab.plot(0.8159,0.3089,'wP',ms=10.5) 

        if name1=="cosmological_parameters--smn" and name2=="cosmological_parameters--omega_m":
            pylab.ylabel(r'$\Omega_m$',fontsize=25)
            pylab.xlabel(r'$\sum m_\nu$ (eV)')
            #pylab.ylim(0.25,0.385)
            pylab.xlim(0.06,1.6)
            pylab.plot(0.06,0.3089,'wP',ms=10.5) 


        if name1=="cosmological_parameters--smn" and name2=="cosmological_parameters--sigma8_input":
            pylab.ylabel(r'$\sigma_8$',fontsize=25)
            pylab.xlabel(r'$\sum m_\nu$ (eV)')
            pylab.xlim(0.06,1.6)
            #pylab.ylim(0.775,0.85)
            pylab.plot(0.06,0.8159,'wP',ms=10.5) 

        if name1=="cosmological_parameters--smn" and name2=="cosmological_parameters--h0":
            pylab.xlabel(r'$\sum m_\nu$ (eV)')
            pylab.ylabel(r'$h$',fontsize=25)
            pylab.xlim(0.06,1.6)
            #pylab.ylim(0.62,0.84)
            pylab.plot(0.06,0.6774,'wP',ms=10.5) 

        pylab.tick_params(axis='both', which='major', size=8., 
                width=1.5, pad=8., direction="in", top=True,right=True)
        pylab.tick_params(axis='both', which='minor', size=5., 
                width=1.5, pad=8., direction="in", top=True,right=True)
...
class CovarianceMatrixEllipse(Plots):
    ...
    def plot_2d(self, name1, i, name2, j):
        ...
        self.plot_cov_ellipse(covmat, pos, nstd=1, facecolor=None, 
            edgecolor=self.line_color(), linewidth=2, fill=False, linestyle='-',label=self.source.label)
        ...
        pylab.xlabel(self.latex(name1))
        pylab.ylabel(self.latex(name2))
	# this is for the fishers without 2sigma and with axes ranges
        #if name1=="cosmological_parameters--omega_m" and name2=="cosmological_parameters--h0":
        #    pylab.xlim(0.25,0.36)
        #    pylab.ylim(0.56,0.8)

        #if name1=="cosmological_parameters--sigma8_input" and name2=="cosmological_parameters--h0":
        #    pylab.xlim(0.805,0.828)
        #    pylab.ylim(0.56,0.8)

        #if name1=="cosmological_parameters--sigma8_input" and name2=="cosmological_parameters--omega_m":
        #    pylab.xlim(0.805,0.828)
        #    pylab.ylim(0.25,0.36)
