...
##datavecname = open("/home/ktanidis/cosmosis/cosmosis-standard-library/likelihood/2pt/cmass_theory.txt") 
##datavec = np.loadtxt(datavecname)

#cov11 = fits.open('/home/ktanidis/Desktop/fsigma8_bsigma8/CMASS_real_covariance_test_chi2/clcov_galaxy_DR12v5_CMASS_zbin_0p45_0p4769_cross_0p45_0p4769_no_dipole.fits')
#cov11p = cov11[0]
##cov11p = cov11p.data[0,7:268,7:268]
##if with ell cuts
#cov11p = cov11p.data[0,7:183,7:183]

#cov21 = fits.open('/home/ktanidis/Desktop/fsigma8_bsigma8/CMASS_real_covariance_test_chi2/clcov_galaxy_DR12v5_CMASS_zbin_0p45_0p4769_cross_0p4769_0p5038_no_dipole.fits')
#cov21p = cov21[0]
##cov21p = cov21p.data[0,:,:]
##cov21p = cov21p.data[0,7:268,7:268]
##if with ell cuts
#cov21p = cov21p.data[0,7:183,7:183]

#cov22 = fits.open('/home/ktanidis/Desktop/fsigma8_bsigma8/CMASS_real_covariance_test_chi2/clcov_galaxy_DR12v5_CMASS_zbin_0p4769_0p5038_cross_0p4769_0p5038_no_dipole.fits')
#cov22p = cov22[0]
##cov22p = cov22p.data[0,:,:]
##cov22p = cov22p.data[0,7:268,7:268]
##if with ell cuts
#cov22p = cov22p.data[0,7:192,7:192]
...
class GaussianLikelihood(object):
    ...
    #def do_likelihood(self, block, config):
        ...
        ##gaussian likelihood for the real covariance
        ##keep11=[0]*261
        ###keep12=[0]*261
        ##keep21=[0]*261
        ##keep22=[0]*261

        ##with ell cuts nell=537
        #keep11=[0]*176
        ##keep12=[0]*261
        #keep21=[0]*176
        #keep22=[0]*185
        ...
        #chi2_11 = np.einsum('i,ij,j', keep11, invcov11, keep11)
        #chi2_21 = np.einsum('i,ij,j', keep21, invcov21, keep21)
        #chi2_22 = np.einsum('i,ij,j', keep22, invcov22, keep22)

        #chi2 = float(chi2_11 + chi2_21 + chi2_22)
    ...
    def build_module(cls):
        ...
        def setup(options):
            ...
            #return (likelihoodCalculator,cuvar)
            return likelihoodCalculator

        def execute(block, config):
            likelihoodCalculator = config
            #(likelihoodCalculator,cuvar) = config
            likelihoodCalculator.do_likelihood(block)
            #likelihoodCalculator.do_likelihood(block, config)
            return 0
